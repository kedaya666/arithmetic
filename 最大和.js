...
var maxSubArray = function(nums) {
    let bigSum = eval(nums.join("+"));
    for(let i=0;i<nums.length;i++){
        let j=i+1;
        while(j<=nums.length){
            let newArr = (j==nums.length?nums.slice(i):nums.slice(i,j));
            let sunNum = eval(newArr.join("+"));
            if(sunNum>bigSum)bigSum = sunNum;
            j=j+1;
        }
    }
    return bigSum;
